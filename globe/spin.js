...
const locations = [
   { lat: 40.7128, lon: -74.0060, name: "New York", image: "images/newyork.jpg", photosPage: "photos/newyork.html" },
   { lat: 51.5074, lon: -0.1278, name: "London", image: "images/london.jpeg", photosPage: "photos/london.html" },
   { lat: 35.6762, lon: 139.6503, name: "Tokyo", image: "images/tokyo.jpeg", photosPage: "photos/tokyo.html" },
   { lat: 43.2965, lon: 5.3698, name: "Marseille", image: "images/marseille.jpeg", photosPage: "photos/marseille.html" },
   { lat: -28.0167, lon: 153.4000, name: "Gold Coast", image: "images/goldcoast.jpeg", photosPage: "photos/goldcoast.html" }
];
...
locations.forEach(loc => {
   const pinGeometry = new THREE.PlaneGeometry(0.05, 0.1); // Adjust size as needed
   const pinMaterial = new THREE.MeshBasicMaterial({ map: pinTexture, transparent: true, side: THREE.DoubleSide });
   const pin = new THREE.Mesh(pinGeometry, pinMaterial);
  
   // Position pin just touching the sphere surface
   const position = latLonToVector3(loc.lat, loc.lon);
   pin.position.copy(position);
  
   // Adjust pin position so it just touches the sphere surface
   pin.position.normalize().multiplyScalar(1.03); // Adjust scalar for touch
  
   pin.lookAt(camera.position); // Ensure the pin faces the camera
   sphere.add(pin);
...
function onMouseClick(event) {
   ...
   if (intersects.length > 0) {
       const intersectedMarker = intersects[0].object;
       const clickedLocation = markers.find(m => m.marker === intersectedMarker).location;
       window.open(clickedLocation.photosPage, '_blank'); // Open the photos page in a new tab
   }
}
...
window.addEventListener('click', onMouseClick); // Add event listener for click events
...
function animate() {
   ...
   controls.update();
   updatePinOrientation(); // Update pin orientation
